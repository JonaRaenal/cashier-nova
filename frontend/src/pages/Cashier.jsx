...
const Cashier = () => {
  ...
  const handleProcessTransaction = async () => {
    ...
    if (Number(paymentAmount) < grandTotal) {
  ...
  return (
    <div className="animate-fade-in -m-6">
      <div className="flex h-[calc(100vh-64px)]">
        ...
        <div className="w-[380px] bg-white border-l border-gray-100 flex flex-col">
          {/* Cart header */}
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-3">
            {items.length === 0 ? (
              ...
              items.map((item) => (
                <div
                  key={item.product_id}
                  className="flex gap-3 p-3 bg-gray-50 rounded-xl animate-slide-in"
                >
                  {/* Thumbnail */}
